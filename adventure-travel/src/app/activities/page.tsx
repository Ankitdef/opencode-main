...
const activities = [
  ...
  },
  {
    ...
    cta: { label: "View Courses", href: "/activities#skiing" },
    color: "sky",
  },
  {
    id: "camping",
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-background">
      ...
      <section id="skiing" className="relative overflow-hidden bg-gradient-to-br from-sky-50 via-blue-50 to-sky-100">
        ...
        {/* Course cards: Skiing · Snowboarding · Backcountry */}
        <div className="relative mx-auto max-w-7xl px-6 pb-16 md:pb-24 lg:px-8">
          <div className="grid gap-6 md:grid-cols-3">
            {skiCourses.map((course, i) => {
              const waHref = `https://wa.me/917817912062?text=${encodeURIComponent(
                `Hi! I'm interested in the ${course.title} in Auli.`
              )}`;
              return (
                <motion.div
                  key={course.type}
                  ...
                  className={`relative overflow-hidden rounded-3xl p-7 shadow-xl transition-transform hover:-translate-y-1 ${
                    course.featured
                      ? "bg-sky-600 text-white shadow-sky-600/25"
                      : "bg-white text-foreground shadow-black/5 ring-1 ring-sky-100"
                  }`}
                >
                  {course.featured && (
                  ...
                  <div className={`inline-flex rounded-2xl bg-gradient-to-br ${course.gradient} p-3 text-white`}>
                    {course.icon}
                  </div>
                  <p className={`mt-4 text-xs font-semibold uppercase tracking-wider ${course.featured ? "text-white/70" : "text-sky-600"}`}>
                    {course.type}
                  </p>
                  <h3 className="mt-1 font-heading text-xl font-bold">{course.title}</h3>
                  <p className={`mt-1 text-sm ${course.featured ? "text-white/60" : "text-muted"}`}>Auli, Uttarakhand</p>
                  <ul className="mt-5 space-y-2.5">
                    {course.includes.map((item) => (
                      <li key={item} className="flex items-start gap-2.5 text-sm">
                        <svg className={`mt-0.5 h-4 w-4 flex-shrink-0 ${course.featured ? "text-white" : "text-sky-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                        </svg>
                        <span className={course.featured ? "text-white/90" : "text-foreground/80"}>{item}</span>
                      </li>
                    ))}
                  </ul>
                  <div className="mt-6 flex items-center justify-between">
                    <div>
                      <p className={`text-xs ${course.featured ? "text-white/60" : "text-muted"}`}>from</p>
                    ...
                    <a
                      ...
                      className={`rounded-xl px-5 py-2.5 text-sm font-semibold transition-all hover:scale-105 ${
                        course.featured ? "bg-white text-sky-600" : "bg-sky-500 text-white"
                      }`}
                    >
                      Book Course
                    </a>
      ...
      <SkiingCoursePopup open={showCourse} onClose={() => setShowCourse(false)} />
    </div>
  );
}

const skiingItinerary = [
...
const skiCourses = [
  {
    ...
    includes: [
      "Certified ski instructor daily",
      "Full kit — skis, boots, poles, helmet",
  ...
  },
  {
    ...
    includes: [
      "Certified snowboard instructor daily",
      "Full kit — board, boots, helmet",
      "Accommodation & all meals",
      "Balance, turning & freestyle basics",
      "Progress tracking",
    ],
  },
  {
    ...
    icon: (
      <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden>
        <path strokeLinecap="round" strokeLinejoin="round" d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM22 12l-5.25 3-1.5-2.25L9.75 12l5.25-3 1.5 2.25L22 12z" />
      </svg>
    ),
    includes: [
      "Avalanche safety & rescue basics",
      "Splitboard / skinning technique",
...
function SkiingCoursePopup({ open, onClose }: { open: boolean; onClose: () => void }) {
  useEffect(() => {
    if (!open) return;
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Skiing and snowboarding course itinerary"
        >
          <motion.div
          ...
          >
            {/* Header */}
            <div className="relative bg-gradient-to-br from-sky-500 to-blue-600 p-6 text-white sm:p-8">
              <button
                onClick={onClose}
                aria-label="Close course itinerary"
              ...
              <p className="text-xs font-semibold uppercase tracking-widest text-sky-100">7-Day Certified Course</p>
              <h3 className="mt-2 font-heading text-2xl font-bold sm:text-3xl">Skiing &amp; Snowboarding in Auli</h3>
              <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1">₹30,000–35,000</span>
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8">
              <p className="text-sm text-muted">
                Available as a <span className="font-semibold text-foreground">skiing</span> or{" "}
                <span className="font-semibold text-foreground">snowboarding</span> course — the same seven-day
                structure, led by certified instructors with all equipment provided.
              </p>

              <h4 className="mt-6 font-heading text-lg font-bold text-foreground">Day-by-Day</h4>
              <ol className="mt-4 space-y-4">
                {skiingItinerary.map((d) => (
                  <li key={d.day} className="flex gap-4">
                    <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-sky-100 font-bold text-sky-700">
                      {d.day}
                    </div>
                    <div>
              ...
              <h4 className="mt-8 font-heading text-lg font-bold text-foreground">What&apos;s Included</h4>
              <ul className="mt-4 grid gap-2 sm:grid-cols-2">
                {skiingInclusions.map((item) => (
                  <li key={item} className="flex items-center gap-2 text-sm text-foreground">
                    <svg className="h-4 w-4 flex-shrink-0 text-sky-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                    </svg>
                    {item}
                  </li>
            ...
            <div className="flex flex-col gap-3 border-t border-gray-100 p-6 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-2xl font-bold text-foreground">
                ₹30,000<span className="text-sm font-normal text-muted"> / 7-day course</span>
              </p>
